import { processHeaders, flatterHeaders, parseHeaders } from '../../src/helpers/headers'
...
describe('helpers:headers', () => {
    ...
    test('should parse headers', () => {
      const parsed = parseHeaders(
        'Content-Type:application/json\r\n' +
          ...
          'Date: Tue, 21 May 2019 09:13:44 GTM\r\n' +
          ':aa\r\n' +
          'key:'
      )
      ...
      expect(parsed['date']).toBe('Tue, 21 May 2019 09:13:44 GTM')
  ...
  describe('processHeaders', () => {
    test('should normalize Content-Type headers name', () => {
      const headers: any = {
    ...
    test('should set not Content-Type if not set and data is  PlainObject', () => {
      const headers: any = {}
  ...
  describe('flatterHeaders', () => {
    ...
    test('should flatter the headers without common headers', () => {
      const headers: any = {
        Accept: 'application/json',
        get: {
          'X-GET-HEADER': 'getHeaderValue'
      ...
      expect(flatterHeaders(headers, 'patch')).toEqual({
        Accept: 'application/json'
      })
    })

    test('should do nothing id headers is undefined or null', () => {
      expect(flatterHeaders(undefined, 'get')).toBeUndefined()
      expect(flatterHeaders(null, 'post')).toBeNull()
    })
  })
})
